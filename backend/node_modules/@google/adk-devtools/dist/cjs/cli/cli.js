...
var import_cli_deploy = require("./cli_deploy.js");
...
function createProgram() {
  ...
  DEPLOY_COMMAND.command("cloud_run").addArgument(AGENT_DIR_ARGUMENT).allowUnknownOption().allowExcessArguments().addOption(PORT_OPTION).option(
    "--project [string]",
    ...
    false
  ).addOption(ORIGINS_OPTION).addOption(VERBOSE_OPTION).addOption(LOG_LEVEL_OPTION).addOption(SESSION_SERVICE_URI_OPTION).addOption(ARTIFACT_SERVICE_URI_OPTION).addOption(COMPILE_AGENT_FILE).addOption(BUNDLE_AGENT_FILE).addOption(AGENT_FILE_MODULE_TYPE).addOption(A2A_OPTION).action(async (agentPath, options) => {
    ...
    try {
      await (0, import_cli_deploy.deployToCloudRun)({
